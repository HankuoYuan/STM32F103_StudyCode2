...
fp32 get_resistor_percent(void) {
    fp32 resistor_percent;
    uint16_t adcx = 0;
    adcx = adcx_get_chx_value(&hadc1, ADC_CHANNEL_8);
    resistor_percent = (fp32) adcx * voltage_vrefint_proportion / 3.3f;
    return resistor_percent;
}

fp32 get_resistor_voltage(void) {
    fp32 resistor_voltage;
    uint16_t adcx = 0;
    adcx = adcx_get_chx_value(&hadc1, ADC_CHANNEL_8);
    resistor_voltage = (fp32) adcx * voltage_vrefint_proportion;
    return resistor_voltage;
}
